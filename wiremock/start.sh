...
if ! [ -e wiremock-standalone.jar ]; then
    ...
    curl https://repo1.maven.org/maven2/com/github/tomakehurst/wiremock-jre8-standalone/2.33.1/wiremock-jre8-standalone-2.33.1.jar -o wiremock-standalone.jar
    status=$?
    if [ ${status} -ne 0 ]; then
